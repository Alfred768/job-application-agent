...
- `job-agent jobs import-sources`：从 `sources.json` 合并多个公开 source，按规范化申请 URL（无 URL 时按公司、职位和地点）去重；重复项保留完整 JD 和全部来源标记。
...
- 当前已实现：生成本地 application package，包含 `review.md`、`jd-analysis.json`、`resume-edit-plan.json`、`submit-gate.txt`、`tailored-resume.md`、`tailored-resume.docx`。
- 当前已实现：从 grounded Markdown tailored resume 草稿导出可上传 DOCX。
- 导出 PDF。
- 检查页数是否为 1 页。
...
- 当前 package exporter 只生成安全的 review artifacts 和新的 tailored resume 文件，不直接改写原始简历。
- PDF 导出和版式质量检查属于下一阶段。
